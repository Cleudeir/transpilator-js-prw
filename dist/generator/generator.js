function genExpr(node) {
    switch (node.type) {
        ...
        case 'ObjectLiteral': {
            const props = node.properties.map(prop => `"${prop.key}" => ${genExpr(prop.value)}`).join(', ');
            return `{ ${props} }`;
        }
        case 'ArrayLiteral': {
...
function genStmt(node, indent = '') {
    switch (node.type) {
        ...
        case 'IfStmt': {
            ...
            const consequent = node.consequent.map(stmt => genStmt(stmt, indent + '    ')).join('\n');
            code += consequent;
            // Generate alternate statements if they exist
            if (node.alternate) {
                code += `\n${indent}Else\n`;
                const alternate = node.alternate.map(stmt => genStmt(stmt, indent + '    ')).join('\n');
                code += alternate;
            }
            code += `\n${indent}EndIf`;
            return code;
